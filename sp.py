...
class SSO(object):
    def __init__(self, sp, environ, start_response, cache=None,
                 wayf=None, discosrv=None, bindings=None, **kwargs):
        self.sp = sp
        self.environ = environ
        self.start_response = start_response
        self.cache = cache
    ...
    def _redirect_to_auth(self, _cli, entity_id, came_from, vorg_name="",
        ...
        return self.response(_binding, ht_args, dont_send=dont_send)
    ...
    def do(self):
        ...
        if done == -1:
            ...
            entity_id = response
            # Do the AuthnRequest
            return self._redirect_to_auth(_cli, entity_id, came_from,
                                          vorg_name="")
...
def authn_context_choice(environ, start_response):
    resp = Response(mako_template="acsrv.mako", template_lookup=LOOKUP,
                    headers=[])
    argv = {"ac": ARGS["authncontext"], "target": "/authnContext"}
...
def application(environ, start_response):
    ...
    if path == "":
        return authn_context_choice(environ, start_response)
...
if __name__ == '__main__':
    ...
    _parser.add_argument('-A', dest='authncontext',
